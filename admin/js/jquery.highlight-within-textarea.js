...
(function ($) {
    ...
    var HighlightWithinTextarea = function HighlightWithinTextarea($el, config) {
        this.init($el, config);
    };
    ...
    HighlightWithinTextarea.prototype = {
        ...
        getType: function getType(instance) {
            var type = typeof instance === 'undefined' ? 'undefined' : _typeof(instance);
            if (!instance) {
                return 'falsey';
        ...
        generate: function generate() {
            this.$el.addClass(ID + '-input ' + ID + '-content').on('input.' + ID, this.handleInput.bind(this)).on('scroll.' + ID, this.handleScroll.bind(this));

            this.$highlights = $('<div>', { class: ID + '-highlights ' + ID + '-content' });

            this.$backdrop = $('<div>', { class: ID + '-backdrop' }).append(this.$highlights);

            this.$container = $('<div>', { class: ID + '-container' }).insertAfter(this.$el).append(this.$backdrop, this.$el) // moves $el into $container
        ...
        fixFirefox: function fixFirefox() {
            // take padding and border pixels from highlights div
            var padding = this.$highlights.css(['padding-top', 'padding-right', 'padding-bottom', 'padding-left']);
            var border = this.$highlights.css(['border-top-width', 'border-right-width', 'border-bottom-width', 'border-left-width']);
            this.$highlights.css({
                'padding': '0',
                'border-width': '0'
            });
        ...
        getRanges: function getRanges(input, highlight) {
        ...
        getArrayRanges: function getArrayRanges(input, arr) {
            var ranges = arr.map(this.getRanges.bind(this, input));
            return Array.prototype.concat.apply([], ranges);
        },

        getFunctionRanges: function getFunctionRanges(input, func) {
            return this.getRanges(input, func(input));
        },

        getRegExpRanges: function getRegExpRanges(input, regex) {
            var ranges = [];
            var match = void 0;
        ...
        getStringRanges: function getStringRanges(input, str) {
            var ranges = [];
        ...
        getRangeRanges: function getRangeRanges(input, range) {
            return [range];
        },

        getCustomRanges: function getCustomRanges(input, custom) {
        ...
        removeStaggeredRanges: function removeStaggeredRanges(ranges) {
        ...
        getBoundaries: function getBoundaries(ranges) {
            var boundaries = [];
            ranges.forEach(function (range) {
                boundaries.push({
                    type: 'start',
        ...
        sortBoundaries: function sortBoundaries(boundaries) {
        ...
        blockContainerScroll: function blockContainerScroll() {
